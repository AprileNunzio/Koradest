...
import { TONI, heroHtml, guidaHtml, campoHtml, leggiCampo, toISODate, AK_STYLES } from './ui_kit.js';
...
export function renderPersonScopedCrudSubapp(el, config) {
    ...
    el.innerHTML = `
        <div class="fade-in-up ak-root" style="--ak-accent:${toneObj.accent}; --ak-soft:${toneObj.soft};${embedded ? ' gap:0.75rem;' : ''}">
            ${embedded ? '' : heroHtml({ title, subtitle, icon, tone })}
            ...
            <section id="crud-kit-records" class="ak-panel" style="display:none;">
                <div class="ak-toolbar">
                    <h3>${recordsTitle} <span class="ak-count" id="crud-kit-count">0</span></h3>
                    <button id="crud-kit-btn-new" class="ak-btn ak-btn-primary" style="padding:0.55rem 1.1rem;"><span class="material-symbols-rounded">add</span>${newLabel}</button>
                </div>
    ...
    function openModal(record = null) {
        ...
        fields.forEach(f => {
            ...
            else if (f.type === 'date') input.value = toISODate(value);

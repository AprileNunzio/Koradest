        ...
        const renderCorpo = async () => {
            ...
            modal.innerHTML = `
                ...
                <div style="display: flex; flex-direction: column; gap: 0.6rem; max-height: 280px; overflow-y: auto;">
                    ${repos.length === 0 ? '<div style="padding: 1.2rem; text-align: center; color: var(--md-on-surface-variant); display: flex; flex-direction: column; align-items: center; gap: 0.5rem;"><span class="material-symbols-rounded" style="font-size: 2rem; opacity: 0.5;">cloud_off</span><div>Tutti i repository sono stati rimossi. Aggiungi una sorgente per scoprire applicazioni disponibili.</div></div>' : ''}
                    ${repos.map(r => `
